...
//        CCLabelTTF *langttf = [CCLabelTTF labelWithString:[NSString stringWithFormat:@" /9        / 4"] fontName:@"DayDream" fontSize:67];
//        langttf.position =  ccp(size.width*0.78, size.height*0.93);
//        [self addChild: langttf z:20];
       ...
       // NSString *displayName = [[NSLocale currentLocale] displayNameForKey:NSLocaleIdentifier value:identifier]; // displayName = “中国
        ...
        NSString *enlang=@"en";
        NSString *chlang=@"zh-Hans";
        
        NSLog(@"\n多语言～～%@\n",identifier);
        
        
        if ([strLanguage isEqualToString:enlang]) {
            CCLOG(@"\ntest多语言\n");
        }
        else if([strLanguage isEqualToString:chlang]){
            CCLOG(@"\n测试多语言\n");
        }
